...
import {DiscoveryState} from './discover.props';
...
import SearchIcon from '../../../theme/assets/images/app/search.svg';
import Animated, {
  useAnimatedStyle,
  useSharedValue,
  withTiming,
...
const DiscoveryScreen: React.FC<Props> = ({
  route,
  navigation,
}): React.JSX.Element => {
  ...
  const settingAnimated = useAnimatedStyle(() => {
    return {
      borderWidth: withTiming(isFocused.value || isFocused.value ? 1 : 0, {
        // duration: 300,
        // easing: Easing.out(Easing.ease),
      }),
      borderColor:
        isFocused.value || isFocused.value ? colors.primary : 'transparent',
    };
  });
  ...
  useEffect(() => {
    ...
    if (route?.params?.reload && route?.params?.numberReload === counter) {
      ...
      const fetchData = async (link: any = linkSelected) => {
        ...
        const filtered = data.filter((item: NewsProps) =>
  ...
  const fetchItem = async (linkSelected: string) => {
    await fetchPublicData(linkSelected).then(async resp => {
      if (resp.articles && resp.articles !== undefined) {
        ...
        const filtered = sliceData.filter((item: NewsProps) =>
          item.title.toLowerCase().includes(searchQuery.toLowerCase()),
        );
        ...
        setNews(filtered);
        await storeAsyncStorage(filtered, type, linkSelected);
      ...
      <View
        ...
        <View
          ...
          <View style={styles.headerContainer}>
            <Text style={styles.headerDiscover}>Discover</Text>

            <Text
              style={{color: colors.secondary, fontSize: moderateScale(12)}}>
              More today's news: {time}
              {/* {counter} {route.params?.numberReload} */}
            </Text>

            <Animated.View style={[styles.searchContainer, settingAnimated]}>
              <SearchIcon
                style={{position: 'absolute', left: moderateScale(10)}}
              />
              <TextInput
